...
import { api, type ExperimentResult, type ResearchFeatureResponse } from '../services/api';
...
export function ResearchPage() {
  ...
  return (<div className="space-y-3">
    ...
    <section className="grid grid-cols-2 gap-2 text-[11px] sm:grid-cols-6">
      ...
      <div className="rounded-md bg-surface-900 p-2 ring-1 ring-surface-700/60">Backtest runs<div className="text-text-secondary">{experiments.length}</div></div>
    ...
    <section className="rounded-lg bg-surface-900 p-3 ring-1 ring-surface-700/70 overflow-x-auto"><h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-text-muted">Experiment Runs (Deterministic Read-only)</h2>{experiments.length === 0 ? <div className="text-xs text-text-secondary">No experiments yet. Placeholder state shown until data is available.</div> : <table className="w-full text-left text-[11px]"><thead className="text-text-muted"><tr><th>experiment id</th><th>strategy</th><th>dataset rows</th><th>Sharpe</th><th>max drawdown</th><th>win rate</th><th>status</th><th>created at</th></tr></thead><tbody>{experiments.map((r)=><tr key={r.experiment_id} className="border-t border-surface-700/50"><td className="py-1.5 font-semibold">{r.experiment_id.slice(0, 12)}</td><td>{r.strategy_name}</td><td>{r.dataset_row_count}</td><td>{r.sharpe}</td><td>{r.max_drawdown_pct}%</td><td>{r.win_rate_pct}%</td><td>{r.status}</td><td>{r.created_at}</td></tr>)}</tbody></table>}<div className="mt-2 text-[10px] text-text-secondary">v0 uses deterministic simulated placeholder backtest logic (no order execution).</div></section>
